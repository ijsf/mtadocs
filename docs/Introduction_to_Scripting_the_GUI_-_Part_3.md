...
[thumb|GUI Teleport Window](/Image:Gui_teleport_tutorial.PNG.md "wikilink")
...
The first thing we need to do is create our GUI elements. For this tutorial we will be using one [window](/Element/GUI/Window.md "wikilink"), three [buttons](/Element/GUI/Button.md "wikilink") and one [label](/Element/GUI/Text_label.md "wikilink"). We will be using **absolute** position values.

As noted in [Previous tutorials](/:Category:GUI_Tutorials.md "wikilink"), all the GUI must be made client side.
...
In this file we will now write a funtion that creates the window. To create a window we will use [guiCreateWindow](/guiCreateWindow.md "wikilink"):
...
Next, we will add the label describing what the buttons do. To create a label we will use [guiCreateLabel](/guiCreateLabel.md "wikilink"):
...
Now we need to add the city teleport buttons. To create a button we will use [guiCreateButton](/guiCreateButton.md "wikilink"):
...
There are several ways this could be done, depending on your personal preference or the particulars of the situation. For this tutorial, we will use a simple [command](/addCommandHandler.md "wikilink").
...
On the clientside, we are triggering the server event “movePlayerToPosition”. So, we will first define that event. To do that we will use [addEvent](/addEvent.md "wikilink"), then [addEventHandler](/addEventHandler.md "wikilink"):
...
Note the use of [spawnPlayer](/spawnPlayer.md "wikilink") rather than, for example, [setElementPosition](/setElementPosition.md "wikilink"). Using [spawnPlayer](/spawnPlayer.md "wikilink") has several advantages, such as being able to set the position when 'de-spawned' or dead, and automatically refilling health. Of course, [setElementPosition](/setElementPosition.md "wikilink") would work just as well to simply move them, so the choice is yours.
