...
**Note that this tutorial builds on content covered in the [GUI Scripting Introduction](/Introduction_to_Scripting_the_GUI.md "wikilink").**
...
The first thing we need to do is create our GUI elements. For this tutorial we will be using one [window](/Element/GUI/Window.md "wikilink"), one [button](/Element/GUI/Button.md "wikilink") and one [label](/Element/GUI/Text_label.md "wikilink"). We will be using **absolute** position values.

As noted in the [Previous tutorial](/Introduction_to_Scripting_the_GUI.md "wikilink"), all the GUI must be made client side.
...
Therefore, we'll write an event handler for [onClientResourceStart](/onClientResourceStart.md "wikilink") to create the window:
...
However, we will also need to show the cursor for the player. This can be done using the same event, [onClientResourceStart](/onClientResourceStart.md "wikilink"), so we can modify the above code to include showing the cursor:
...
When the player clicks on any part of the GUI, the event "[onClientGUIClick](/onClientGUIClick.md "wikilink")" will be triggered for the GUI component you clicked on. This allows us to easily detect any clicks on the GUI elements we want to use. For example, we can attach the event to the 'rulesButton' button to catch any clicks on it:
...
Now that we can detect when the player clicks on the button, we need to write code to manage what happens when they do. In our [onClientGUIClick](/onClientGUIClick.md "wikilink") event handle, we told it to call the function 'acceptRules' whenever 'rulesButton' is clicked. Therefore, we can now use the function 'acceptRules' to control what happens when the button is clicked:
...
To combat this, we will add a timer using [setTimer](/setTimer.md "wikilink") to kick them from the server after a period of time if they have not accepted the rules. First, we will need a global variable to store the timer pointer:
...
Put this [setTimer](/setTimer.md "wikilink") line at the bottom of your 'createRulesWindow' function. This will create a timer that will trigger after 30 seconds.
...
Note the use of [triggerServerEvent](/triggerServerEvent.md "wikilink") to call the server. You should have experience with client-server interaction from the [Previous tutorial](/Introduction_to_Scripting_the_GUI.md "wikilink"). If not, you can go back and read the full explanation there.
...
Note the use of [addEvent](/addEvent.md "wikilink") and [addEventHandler](/addEventHandler.md "wikilink") on the server. You should have experience with client-server interaction from the [Previous tutorial](/Introduction_to_Scripting_the_GUI.md "wikilink"). If not, you can go back and read the full explanation there.
...
**Note that [kickPlayer](/kickPlayer.md "wikilink") will require your resource to have kick access in your ACL.**
...
For more information on the [ACL](/ACL.md "wikilink"), see the [ACL](/ACL.md "wikilink") wiki page.

That concludes this tutorial. You should now have a fully working rules window for your server.

For further help with GUI, see the [GUI tutorials](/:Category:GUI_Tutorials.md "wikilink").
